# -*- coding: utf-8 -*-
"""
简化版本 - 直接提取September数据,不修改load_data.py
Simplified version - directly extract September data
"""
...
def extract_and_save(name, filepath):
    ...
    try:
        ...
        output_lines.append(df_anno.head(20).to_string())
        output_lines.append("\n")
        ...
        output_lines.append(f"\n===== FIRST 30 EVENTS =====\n")
        output_lines.append(df_events[['time_sec', 'label']].head(30).to_string())
        output_lines.append("\n")

        # 事件计数
        output_lines.append(f"\n===== EVENT COUNTS =====\n")
        event_counts = df_events['label'].value_counts()
        output_lines.append(event_counts.to_string())
        output_lines.append("\n")
        ...
        print(f"  ✓ Size: {len(output_text)} bytes")
...
def main():
    print("="*100)
    print("SEPTEMBER EEG DATA EXTRACTION - SIMPLIFIED VERSION")
    print("="*100)

    success = 0
    failed = 0

    for name, filepath in SEPTEMBER_FILES.items():
        if extract_and_save(name, filepath):
            success += 1
        else:
            failed += 1

    print("\n" + "="*100)
    print(f"COMPLETE: {success}/5 successful, {failed}/5 failed")
    print("="*100)
